...
extern "C"
{
    ...
    // 如果是180VR,设置图像渐变过程
    void VRSoft_SetDrawModeCartoon(VRHANDLE hVR, XMVR180DrawMode fromMode, XMVR180DrawMode toMode, int step, int total);
    ...
    void VRSoft_SetFecParams(VRHANDLE hVR, int xCenter, int yCenter, int radius, int imgWidth, int imgHeight);
    ...
    // 通用型数据设置接口
    void VRSoft_SetTexture(VRHANDLE hVR, XMVRPixelFormat format, unsigned char* pData, int len, int width, int height);
    ...
    void VRSoft_Multi_SetRGBTexture(VRHANDLE hVR, int gridId, unsigned char* pData, int len, int width, int height);

    // 设置YUV420格式的图像数据(仅多画面拼接可用)
    void VRSoft_Multi_SetYUV420PTexture(VRHANDLE hVR, int gridId, unsigned char* pData, int len, int width, int height);

    // 通用型数据设置接口(仅多画面拼接可用)
    void VRSoft_Multi_SetTexture(
        VRHANDLE hVR, int gridId, XMVRPixelFormat format, unsigned char* pData, int len, int width, int height);
    ...
    // 输入新的YUV数据，指定窗口索引
    void VRSoft_SetEGLImageKHR_YUVIn(VRHANDLE hVR, int gridId, EGLImageKHR* pImgHKRIn, int width, int height);
    // 设置YUV输出内存,一般只调用一次即可
    void VRSoft_SetEGLImageKHR_YUVOut(VRHANDLE hVR, EGLImageKHR* pImgHKROut);
    ...
    // for 360VR
    void VRSoft_SetPTZ(VRHANDLE hVR, double pan, double tilt, double zoom, double roll, double wallAngle);
    void VRSoft_GetPTZ(VRHANDLE hVR, double* pOutPan, double* pOutTilt, double* pOutZoom, double* pOutRoll);
    // 分屏/多屏处理时使用以下接口
    void VRSoft_SetPTZ_SubScreen(
        VRHANDLE hVR, int screenIndex, double pan, double tilt, double zoom, double roll, double wallAngle);
    void VRSoft_GetPTZ_SubScreen(
        VRHANDLE hVR, int screenIndex, double* pOutPan, double* pOutTilt, double* pOutZoom, double* pOutRoll);

    // for 180VR
    // zoom : 0.25 - 1.0 - 20.0
    void VRSoft_SetRotateZoom(VRHANDLE hVR, double rotateX, double rotateY, double rotateZ, double zoom);
 ...
 * @param zoom
 */
    void VRSoft_SetRotateZoomRecoverMode(VRHANDLE hVR, double rotateX, double rotateY, double rotateZ, double zoom);

    void
    VRSoft_GetRotateZoom(VRHANDLE hVR, double* pOutRotateX, double* pOutRotateY, double* pOutRotateZ, double* pOutZoom);

    void VRSoft_SetRotateZoom_SubScreen(
        VRHANDLE hVR, int screenIndex, double rotateX, double rotateY, double rotateZ, double zoom);
    void VRSoft_GetRotateZoom_SubScreen(
        VRHANDLE hVR, int screenIndex, double* pOutRotateX, double* pOutRotateY, double* pOutRotateZ, double* pOutZoom);

    void VRSoft_GetPTZFor180VR(VRHANDLE hVR, double* rotateX, double* rotateY, double* rotateZ, double* zoom);

    void VRSoft_SetPTZFor180VR(VRHANDLE hVR, double rotateX, double rotateY, double rotateZ, double zoom);
    //翻转
    void VRSoft_SetRotationFor180VR(VRHANDLE hVR, double rotation);
 ...
 * @param zoomValue
 */
    void VRSoft_SetLogicZoom(VRHANDLE hVR, double zoomMin, double zoomMax, double zoomDiv, double zoomValue);

    float VRSoft_GetLogicZoom(VRHANDLE hVR, double zoomMin, double zoomMax);
    ...
    void VRSoft_DisplayRect(VRHANDLE hVR, int x0, int y0, int x1, int y1, int width, int height);

    // 将指定目标点显示在屏幕可见区域(屏幕中心正上方)
    // 为了同时兼容不同分辨率,传入顶点坐标的同时传入相对应的窗口(视频)分辨率
    // (x0, y0) (x1, y1)是相对于xSize/ySize的坐标
    void VRSoft_DisplayTarget(VRHANDLE hVR, int targetX, int targetY, int width, int height);
