...
- (UIButton *)btnThumbnail{
    if (!_btnThumbnail) {
        _btnThumbnail = [[UIButton alloc] initWithFrame:CGRectMake(5, 65, 40, 30)];
    }
    
    return _btnThumbnail;
}
...
- (UILabel *)devType {
    if (!_devType) {
        _devType = [[UILabel alloc] initWithFrame:CGRectMake(60, 50, 80, 30)];
...
- (UILabel *)devSN {
    if (!_devSN) {
        _devSN = [[UILabel alloc] initWithFrame:CGRectMake(60, 80, 80, 30)];
...
- (void)configSubView {
    ...
    [self.contentView addSubview:self.btnThumbnail];
...
#pragma mark - setter
- (void)setDeviceStatus:(XMDeviceStatus)deviceStatus {
    switch (deviceStatus) {
        case XMDeviceStatusSleep:
            _onlineState.image = [UIImage imageNamed:@"ic_sleep"];
            break;
        case XMDeviceStatusWake:
            _onlineState.image = [UIImage imageNamed:@"ic_wake_green"];
            break;
        case XMDeviceStatusPrepareSleep:
            _onlineState.image = [UIImage imageNamed:@"Prepare_sleep"];
            break;
        case XMDeviceStatusDeepSleep:
            _onlineState.image = [UIImage imageNamed:@"sleepnotwakeup"];
            break;
        case XMDeviceStatusOnline:
            _onlineState.image = [UIImage imageNamed:@"online"];
            break;
